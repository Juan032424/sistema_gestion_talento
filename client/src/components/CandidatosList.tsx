...
const CandidatosList: React.FC = () => {
    ...
    const handleExportCSV = () => {
        ...
        const headers = [
            "ID", "Nombre", "Email", "Teléfono", "Vacante", "Etapa Actual", "Fuente", 
            "Estado Entrevista", "Postulado El", "Resultado", "Score Técnico"
        ].join(",");
        ...
        const rows = candidatos.map(c => {
            return [
                ...
                `"${c.resultado_candidato || ''}"`,
                `${c.calificacion_tecnica || 0}`
            ].join(",");
        });
        ...
        link.setAttribute("download", `candidatos_export_${new Date().getTime()}.csv`);
    ...
    const handleSelectCandidato = async (id: number) => {
        try {
            const res = await api.get(`/candidatos/${id}`);
            setSelectedCandidato(res.data);
        } catch (error) {
            console.error("Error loading full candidate detail", error);
    ...
    const columns: ColumnDef<Candidato>[] = [
        ...
        {
            header: "Etapa",
            accessorKey: "etapa_actual",
            sortable: true,
            cell: (c) => (
                <span className="px-2 py-1 rounded bg-[#055098]/10 text-blue-500 text-[10px] font-bold uppercase tracking-tight">
                    {c.etapa_actual}
                </span>
            )
        },
        {
            header: "Fuente",
            accessorKey: "fuente_reclutamiento",
            sortable: true,
            cell: (c) => <span className="text-xs text-gray-400 font-medium">{c.fuente_reclutamiento}</span>
        },
        {
            header: "HV / CV",
            id: "cv",
            cell: (c) => c.cv_url ? (
                <button 
                    ...
                    className="px-2 py-1 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 rounded-lg text-[10px] font-bold transition-all"
                >
                    Ver CV
                </button>
        ...
        {
            header: "Etdo. Entrevista",
            accessorKey: "estado_entrevista",
            sortable: true,
            cell: (c) => (
                <span className={cn(
        ...
        },
        {
            ...
            sortable: true,
            className: "text-xs",
            cell: (c) => <span className="text-gray-500">{c.fecha_entrevista ? new Date(c.fecha_entrevista).toLocaleDateString() : '—'}</span>
        },
        {
            header: "Resultado",
            accessorKey: "resultado_candidato",
            sortable: true,
            cell: (c) => (
                <span className={cn(
                    "px-2 py-1 rounded text-[9px] font-black uppercase tracking-tighter",
        ...
        {
            header: "Motivo No Apto",
            accessorKey: "motivo_no_apto",
            sortable: true,
            cell: (c) => <span className="text-xs text-gray-500 truncate block max-w-[150px]">{c.motivo_no_apto || '—'}</span>
        },
        {
            header: "Score Técnico",
            accessorKey: "calificacion_tecnica",
            sortable: true,
            cell: (c) => (
        ...
        {
            header: "AI Match (%)",
            accessorKey: "score_tecnico_ia",
            sortable: true,
            cell: (c) => (
                <div className="flex flex-col">
                    <div className="flex items-center gap-1">
                        <Zap size={10} className="text-blue-400" />
                        <span className="text-xs font-black text-blue-400">{(c as any).score_tecnico_ia || '0'}%</span>
                    </div>
                    <div className="w-16 bg-white/5 h-1 rounded-full mt-1 overflow-hidden">
                        <div className="bg-[#055098] h-full" style={{ width: `${(c as any).score_tecnico_ia || 0}%` }} />
                    </div>
                </div>
            )
        },
        {
            header: "Analítica 90D",
            accessorKey: "estatus_90_dias",
            sortable: true,
            cell: (c) => <span className="text-xs text-gray-400 font-medium">{c.estatus_90_dias || '—'}</span>
        },
        {
            header: "Sentiment",
            accessorKey: "offboarding_sentiment",
            sortable: true,
            cell: (c) => (c as any).offboarding_sentiment ? (
                <span className={cn(
                    "px-2 py-1 rounded text-[9px] font-black uppercase tracking-tighter",
        ...
        },
        {
            ...
            cell: (c) => (
                <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => handleSelectCandidato(c.id)} className="p-2 text-gray-500 hover:text-white hover:bg-white/5 rounded-lg transition-all"><Eye size={16} /></button>
                    {canEdit && (
                        <button onClick={() => handleCandidatoEdit(c)} className="p-2 text-gray-500 hover:text-blue-500 hover:bg-white/5 rounded-lg transition-all"><Edit2 size={16} /></button>
                    )}
                </div>
            )
        }
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {canEdit && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-[#055098] text-white rounded-xl text-xs font-bold hover:bg-blue-600 transition-all shadow-lg shadow-[#055098]/20"
                        >
                            <Plus size={14} />
                            Registrar Candidato
                        </button>
            ...
            <DataTable 
                ...
                columns={columns} 
                searchPlaceholder="Buscar candidatos por nombre, vacante o etapa..."
                onRowClick={(c) => handleSelectCandidato(c.id)}
                emptyMessage="No hay candidatos registrados para seguimiento."
                rowClassName={(c) => {
                    const isHighlighted = highlightId && String(c.id) === String(highlightId);
                    return isHighlighted ? "bg-blue-600/20 border-l-4 border-blue-500 relative z-10" : "";
                }}
            />
                ...
                {selectedCandidato && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="relative bg-slate-900 border border-white/10 rounded-3xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-[80vh] md:h-auto overflow-y-auto custom-scrollbar"
                        >
                            {/* Left Side: Avatar \u0026 Basic Info */}
                            <div className="p-8 bg-white/[0.02] border-r border-white/5 md:w-80 flex flex-col items-center">
                                <div className="w-24 h-24 bg-gradient-to-tr from-[#055098] to-blue-600 rounded-3xl flex items-center justify-center text-3xl font-black text-white shadow-2xl shadow-blue-500/20 mb-6">
                                ...
                                <h3 className="text-xl font-black text-white text-center mb-2">{selectedCandidato.nombre_candidato}</h3>
                                 <div className="flex items-center gap-2 mb-4">
                                     ...
                                     <span className="text-sm font-black text-blue-400 uppercase tracking-widest">Score IA: {(selectedCandidato as any).score_tecnico_ia || '0'}%</span>
                                 </div>
 
                                 <div className="w-full mb-8">
                                     {selectedCandidato.cv_url ? (
                                         <button
                                             onClick={() => { setSelectedCandidato(null); setCvModalUrl(selectedCandidato.cv_url); }}
                                             className="flex items-center justify-center gap-2 w-full py-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-emerald-400 text-xs font-black uppercase tracking-widest hover:bg-emerald-500/20 transition-all active:scale-95 shadow-xl shadow-emerald-500/10"
                                         >
                                             <FileText size={16} />
                                             <span>Ver Hoja de Vida</span>
                                         </button>
                                     ) : (
                                         <div className="flex items-center justify-center gap-2 w-full py-4 bg-white/5 border border-white/5 rounded-2xl text-gray-600 text-[10px] font-bold uppercase tracking-widest">
                                             <FileText size={16} />
                                ...
                                <div className="w-full space-y-4">
                                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                                        ...
                                        <p className="text-sm font-bold text-blue-500">{selectedCandidato.etapa_actual}</p>
                                    </div>
                                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                                ...
                                <button
                                    ...
                                    className="mt-auto w-full py-4 bg-slate-800 border border-white/10 rounded-2xl text-white text-xs font-black uppercase tracking-widest hover:bg-white/5 transition-all active:scale-95"
                                >
                                    Editar Perfil
                                </button>
                            </div>
                            ...
                            <div className="flex-1 p-8 overflow-y-auto">
                                <div className="flex justify-between items-start mb-8">
                                    <div>
                                        ...
                                        <p className="text-lg font-bold text-white">Perfil Profesional y Aplicación</p>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                                    <DetailBox icon={<User size={16} />} label="Identificación" value={`${selectedCandidato.tipo_identificacion || 'C.C.'} ${selectedCandidato.cedula || 'N/A'}`} />
                                    ...
                                    <DetailBox icon={<MapPin size={16} />} label="Dirección" value={selectedCandidato.direccion || 'No disponible'} />
                                    <DetailBox icon={<GraduationCap size={16} />} label="Educación" value={selectedCandidato.nivel_educativo || 'No especificada'} />
                                    <DetailBox icon={<Briefcase size={16} />} label="Experiencia" value={`${selectedCandidato.experiencia_total_anos || 0} años`} />
                                    <DetailBox icon={<Users size={16} />} label="Estado Civil / Género" value={`${selectedCandidato.estado_civil || 'N/A'} - ${selectedCandidato.genero || 'N/A'}`} />
                                ...
                                <div className="mb-12">
                                    <h4 className="text-[10px] uppercase font-black text-blue-500 tracking-widest mb-4">Vacante Vinculada</h4>
                                    <div className="p-6 bg-slate-800 rounded-3xl border border-white/5 relative overflow-hidden group">
                                ...
                                <div>
                                    <h4 className="text-[10px] uppercase font-black text-emerald-500 tracking-widest mb-4">Feedback & Notas de IA</h4>
                                    <div className="p-6 bg-emerald-500/[0.03] border border-emerald-500/10 rounded-3xl">
                                        <p className="text-sm line-height-relaxed text-gray-400 italic">
                                            {selectedCandidato.resumen_ia_entrevista || "Evaluación preliminar completada. El candidato muestra interés en el cargo y cumple con los requisitos mínimos de experiencia académica."}
                                        </p>
                                    </div>
                                </div>
                            </div>
